...
identifier = f'poisson_nodaytype_{season}'  # identifier calibration
rundate = '2024-09-12'

# scenario settings
N = 10                                      # number of repeated simulations
vaccine_rate_modifiers = [0.8, 1, 1.2]      # relative rates of vaccination
...
samples_dict = json.load(open(f'../data/interim/calibration/{season}/poisson_nodaytype_{season}_SAMPLES_{rundate}.json'))
...
output = []
for vaccine_rate_modifier in vaccine_rate_modifiers:
    # alter parameter value
    model.parameters['vaccine_rate_modifier'] = vaccine_rate_modifier
    # simulate model
...
# overall
for i in range(len(vaccine_rate_modifiers)):
