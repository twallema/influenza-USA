...
hierarchal_beta = True              # Hierarchal structure on transmission rate with L1 regression
...
model = initialise_SVI2RHD(spatial_resolution=sr, age_resolution=ar, season=season, hierarchal_beta=hierarchal_beta, distinguish_daytype=dd,
                           stochastic=stoch, start_sim=start_calibration)
...
if __name__ == '__main__':
    ...
    pars = ['rho_h', 'f_I', 'beta_US',
            'delta_beta_Dec', 'delta_beta_Jan', 'delta_beta_Feb', 'delta_beta_Mar',
            'delta_beta_states', 'f_R']
    # labels in output figures
    labels = [r'$\rho_h$', r'$f_I$', r'$\beta_{US}$',
                r'$\Delta \beta_{Dec}$',  r'$\Delta \beta_{Jan}$',  r'$\Delta \beta_{Feb}$',  r'$\Delta \beta_{Mar}$',
                r'$\Delta \beta_{state}$', r'$f_R$']
    # parameter bounds
    bounds = [(1e-9,0.01), (1e-8,1e-3), (0.01,0.060),
              (-0.5,0.5), (-0.5,0.5), (-0.5,0.5), (-0.5,0.5),
              (-0.5,0.5), (0.01,0.99)]
    # priors
    log_prior_prob_fcn = [
        log_prior_uniform, log_prior_uniform, log_prior_uniform,
        log_prior_normal_L2, log_prior_normal_L2, log_prior_normal_L2, log_prior_normal_L2,
        log_prior_normal_L2, log_prior_uniform,
    ]
    stdev = 0.05
    L1_weight = 0.40
    ...
    # Initial guess
    if not backend_path:
        theta = [rho_h, f_I, beta_US, delta_beta_Dec, delta_beta_Jan, delta_beta_Feb, delta_beta_Mar] + (n_states+1)*[delta_beta_states,] + (n_states+1)*[f_R,] 
