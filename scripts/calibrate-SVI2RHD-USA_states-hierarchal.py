...
## continue run
run_date = '2024-10-22'                                                         # First date of run
...
# temporary code
#theta = list(theta)
#theta = theta[0:4] + [365/np.log(2),] + theta[4:4+2*n_regions+n_temporal_modifiers] + list(1e-2*np.ones(n_regions)) + theta[4+2*n_regions+n_temporal_modifiers:]
# >>>>>>>>>>>>>>>>>>>>>>>>>
...
if __name__ == '__main__':
    ...
    # parameter bounds
    bounds = [(1e-9,0.01), (1e-8,1e-3), (0.01,0.060), (0.10,0.90), (365/2/np.log(2), 2000),
    ...
    log_prior_prob_fcn_args = [
        # top level
        bounds[0], bounds[1], bounds[2], bounds[3], (2, np.log(2)/365*2, 365/2/np.log(2)),
        # mid level
    ...
    # Initial guess
    if not backend_path:
        theta = [rho_h, f_I, beta_US, f_R, T_r] + n_regions*[delta_beta_regions,] + n_temporal_modifiers*[delta_beta_temporal,] + \
