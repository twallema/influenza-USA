...
from influenza_USA.SIR_SequentialTwoStrain.hierarchical_objective_functions import log_posterior_probability, dump_sampler_to_xarray, traceplot, plot_fit, hyperdistributions
...
samples_path = ''
print_n = 1
backend = 'test_BACKEND_2025-01-29.hdf5'
discard = 9000
thin = 10
...
pars_model_0 = pd.read_csv('calibrated_parameters.csv', index_col=0)[seasons]
    ...
    with get_context("spawn").Pool(processes=16) as pool:
        ...
        for sample in sampler.sample(pos, iterations=max_n, progress=True, store=True, skip_initial_state_check=True):
            ...
            if sampler.iteration % print_n:
                ...
                # ..dump samples
                samples = dump_sampler_to_xarray(sampler.get_chain(discard=discard, thin=thin), hyperpars_shapes, pars_model_shapes, seasons)
