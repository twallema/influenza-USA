...
library(readxl)
data_long <- read.csv("../interim/mobility/mobility_commuters_2016-2020_longform.csv", colClasses = c(origin = "character", destination = "character")) 
...
```{r save}
write.csv(M_hatRad, "../interim/mobility/matrix_radiation_counties.csv")
```
...
# visualise transform
hist(x=data_transform, breaks=30, xlab = "log10 normalised commutes", probability=TRUE, main = "Comparison data-model", xlim=c(-12,1), ylim = c(0, 1))
...
# save data
write.csv(matrix_df, "../interim/mobility/matrix_radiation_states.csv", row.names=FALSE)
```