...
# step 3: correct for completely new Connecticut FIPS codes 
## step 3a: compute average demo-weighted fraction of Connecticut population making a trip outside home county
...
## step 3b: compute average demo-weighted fraction of Connecticut population making a trip inside home county
...
# step 3c: fill in trips originating from Connecticut to non-Connecticut counties
...
for fips in out[out['state_o'] == '09']['origin'].unique():             # loop over post-2020 Connecticut codes
    out.loc[((out['origin'] == fips) & (out['destination'] == fips)), 'commuters'] = (f_ondiagonal * demography.loc[fips]).values # fill in on-diagonal trips
    out.loc[((out['origin'] == fips) & (out['destination'] != fips)), 'commuters'] = np.ones(len(out.loc[((out['origin'] == fips) & (out['destination'] != fips)), 'commuters'])) * \
                                                                                        (f_offdiagonal * demography.loc[fips]).values / \
                                                                                            len(out.loc[((out['origin'] == fips) & (out['destination'] != fips)), 'commuters']) # spread off-diagonal uniformly over all possible destinations  (does not influence outcome of departure-diffusion radiation model)
# step 3d: fill in all non-Connecticut originating trips to Connecticut counties
...
# step 4: correct for splitting of '02261' --> '02063' and '02066'
# step 4a: fill in trips originating from former '02261' 
...
for fips in ['02063', '02066']:
    ...
    # fill in off-diagonal trips
    out.loc[((out['origin'] == fips) & (out['destination'] != fips)), 'commuters'] = np.ones(len(out.loc[((out['origin'] == fips) & (out['destination'] != fips)), 'commuters'])) * data[((data['county_o'] == '02261') & (data['county_d'] != '02261'))]['pop_flows'].sum() * r_pop.loc[fips] / len(out.loc[((out['origin'] == fips) & (out['destination'] != fips)), 'commuters'])
# step 4b: distribute all non-'02261' originating trips to former '02261'
...
# step 7: check if there are counties with more off-diagonal trips than inhabitants
