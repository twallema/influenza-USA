...
library(readxl)
data_long <- read.csv("../../interim/mobility/intermediates/to_county_data/mobility_commuters_2016_2020_counties_longform.csv", colClasses = c(origin = "character", destination = "character")) 
...
```{r fit}
modRad <- mobility(data=matrix_list, model='radiation', type='basic', DIC=TRUE, parallel=TRUE, n_chain=2, n_burn=2, n_samp=2, n_thin=1)
```
...
```{r save}
write.csv(M_hatRad, "../../interim/mobility/fitted_models/to_county_data/matrix_counties.csv")
```
...
# save data
write.csv(matrix_df, "../../interim/fitted_models/to_county_data/mobility/matrix_states.csv", row.names=FALSE)
```